...
public sealed class OpenTableDetailPage : ContentPage
{
    ...
    private View BuildHeader()
    {
        ...
        {
            Text = $"{Safe(_table.TableName)} • {RevenueApiClient.Money(_table.Total)}",
    ...
    {
        var items = _table.Items ?? new List<OpenTableItemDto>();
        ...
        if (items.Count == 0)
        {
            ...
            {
                Text = "Chưa có chi tiết món. Cần đồng bộ Mobile Server và Worker phiên bản mới để hiển thị dữ liệu món thật.",
        ...
        content.Children.Add(new Grid
        {
            ...
            Children =
            {
                ...
                new Label
                {
                    Text = RevenueApiClient.Money(_table.Total),
    ...
    private static Grid BuildItemsGrid(IReadOnlyList<OpenTableItemDto> items)
    {
        var grid = new Grid
        {
            ColumnSpacing = 0,
...
internal static class ViewColumnExtensions
{
    public static T AtColumn<T>(this T view, int column) where T : BindableObject
    {
